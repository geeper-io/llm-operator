...
llm-operator-image-bundle.tar: docker-build
	docker save ${IMG} -o llm-operator-image-bundle.tar
...
.PHONY: install.yaml
install.yaml: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
